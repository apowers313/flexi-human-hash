...
function _create(name, opts) {
    ...
    // TODO: check args
    const dictCreate = dictRegistry.get(name);
    if (!dictCreate) {
        throw new Error(`couldn't find dictionary: '${name}'`);
    }

    // create the new dictionary
    const newDict = dictCreate(opts);
...
function _unhash(_name, _opts) {
    ...
    const bits = entropyBits(dict.size);
    const rndMax = 2 ** bits;
    const word = this._words[this._id - 1];
    const value = dict.reverseLookup(word) % rndMax;
    if (value === -1) {
...
function _calcEntry(rndSrc, dictSz) {
    const numBits = entropyBits(dictSz);
    const rndMax = 2 ** numBits;
    const overflowEntries = dictSz % rndMax;
