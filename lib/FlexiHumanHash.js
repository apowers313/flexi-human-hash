...
class FlexiHumanHash {
    ...
    hash(data, opts = {}) {
        this._currentData = new RandomSource(data);
...
function _handlebarHelper(name, ... args) {
    ...
    let dict = this.dictMap.get(name);
    let dictEntry = _calcEntry(this._currentData, dict.size);
    let word = dict.getEntry(dictEntry);
    opts.args.forEach((a) => {
...
function _create(name, opts) {
    ...
    console.log("newDict.size", newDict.size);
    console.log("entropyBits(newDict.size)", entropyBits(newDict.size));
    this.entropyBits += entropyBits(newDict.size);
    this.dictMap.set(name, newDict);
}
