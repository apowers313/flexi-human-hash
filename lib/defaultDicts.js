...
class DecimalDict extends FlexiDict {
    ...
    reverseLookup(str) {
        ...
        if (isNaN(n) || n < 0 || n >= this.size) {
            return -1;
        }
        // Verify the string representation matches (handles leading zeros, etc.)
        if (n.toString() !== str) {
            return -1;
        }
        return n;
...
class HexDict extends FlexiDict {
    constructor(opts = {}) {
        ...
        if (this.nibbles < 1 || this.nibbles > nibblesMax) {
            throw new TypeError(`hex must be between 1 and ${nibblesMax} nibbles, got ${this.nibbles}`);
        }
    }
    ...
    reverseLookup(str) {
        ...
        // Verify the string representation matches (handles leading zeros, case differences, etc.)
        if (n.toString(16) !== str.toLowerCase()) {
            return -1;
        }
        return n;
    }
